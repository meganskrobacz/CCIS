...
class Classic(tools.States):
    def __init__(self, screen_rect, difficulty, markov_states): 
        ...
        self.ai = AI.AIPaddle(self.screen_rect, self.ball.rect, difficulty, markov_states)
    ...
    def movement(self, keys, player_direction):
        if self.ai.move_up:
            self.paddle_left.move(0, -1)
        if self.ai.move_down:
            self.paddle_left.move(0, 1)
        if self.ai.index_of_chosen_markov_state is not -1:
            # determine the "weight", i.e. the speed at which to move
            # If it's far away, move faster
            # Otherwise, move slower
            rect = self.markov_states[self.ai.index_of_chosen_markov_state][1]
            distance = abs(self.paddle_left.rect.centery - rect.centery)
            weight = 1 + ((distance * self.markov_states[self.ai.index_of_chosen_markov_state][0]) / 10000)
            if self.ai.move_up_markov:
                self.paddle_left.move(0, -1, weight)
            if self.ai.move_down_markov:
                self.paddle_left.move(0, 1, weight)
        
        self.paddle_right.move(0, player_direction)
    ...
    def update(self, now, keys, player_direction):
        if not self.pause:
            self.ai.update(self.ball.rect, self.ball, self.paddle_left.rect)
            self.markov_states = self.ai.markov_states
            ...
            self.movement(keys, player_direction)
        else:
            self.pause_text, self.pause_rect = self.make_text("PAUSED",
